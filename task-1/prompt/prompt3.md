# SCADA Data Aggregation and Operations Support Service - v3
...
Implement a telemetry ingestion module. Each reading MUST include: `well_id` (String), `facility_id` (String), `timestamp` (ISO-8601 String), `channel` (Enum: `pressure`, `temperature`, `flow`, `gor`), `value` (Double), `transport` (Enum: `cellular`, `satellite`, `radio`), and `quality_flag` (Enum: `ok`, `flatline`). The system MUST distinguish `flatline` (value unchanged for exactly 5 consecutive readings). 
Additionally, implement a gap-detection query model that infers `signal_loss` when no reading is received for exactly 2 times the expected transport interval (2 minutes for radio, 30 minutes for cellular, 120 minutes for satellite). The system MUST attribute the cause: if multiple wells sharing the same `facility_id` and `transport` drop simultaneously, classify as `comms_outage`; if isolated, classify as `sensor_failure`. Store readings in an append-only `telemetry` table designed to preserve raw history indefinitely, and availability events in a `telemetry_status` table.

**R02 — Sample Dataset**
Include a bundled sample dataset generator. It MUST populate the database with: exactly 50 wells mapped to exactly 10 facilities, exactly 3 sales delivery points, exactly 1 gathering system, exactly 10 equipment inventory items, exactly 5 lease assignments, and exactly 30 days of telemetry history. The generator MUST produce mixed sampling cadences based on transport: radio MUST use 1-minute intervals, cellular MUST use 15-minute intervals, and satellite MUST use 60-minute intervals. The generator MUST execute automatically on first startup if the database is empty.
