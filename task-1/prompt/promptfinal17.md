...
Implement an append-only `telemetry` table AND a `channel_status` table. A background coroutine MUST run on startup AND repeat every 60 seconds performing: (1) Flatline check — fetch the last 5 readings per `(well_id, channel)`; if all 5 values are identical, set `signal_condition = "potential_flatline"`. (2) Outage check — if `current_time - last_seen > 2 * expected_cadence_minutes`, mark `connectivity_condition = "overdue"`; group overdue channels by `(facility_id, transport)`: if count ≥ 3 set `probable_cause = "probable_transport_path_outage"`, otherwise set `probable_cause = "isolated_local_fault"`.

**R02 — Scale & Sample Data**
The `SampleDataGenerator` MUST insert exactly 200 wells AND 40 facilities (5 wells/facility). Transport cadence bands: wells 1–80 → radio/1 min, 81–160 → cellular/15 min, 161–200 → satellite/60 min. **Data Horizon:** Seed 72 hours of telemetry strictly for bootstrap/demo purposes, but MUST seed at least **31 days of alarm history** to satisfy suppression rules. Use a fixed seed for reproducibility.
...
5. `sales_meter_volume`: If `Σ net_oil_at_point > 0`, set `sales_meter_volume = meter_reading * (well_net_oil / Σ net_oil_at_point)`. If `Σ net_oil_at_point <= 0`, set `sales_meter_volume = 0`.
6. The `allocationStatus` MUST be set to `"unallocatable"` if `Σ net_oil_at_point <= 0`, otherwise it MUST be set to `"allocated"`.
...
`GET /reports/regulatory`: Generate reports. (1) `federal_ghg` — methane AND CO2e from `net_gas_volume` with `calculation_basis` sourced from the allocation `source_flag`. (2) `state_production` — per-well totals. (3) `blm_lease` — totals grouped by `lease_id`. **Security Constraint:** You MUST derive accessible organizations from the authenticated user context server-side rather than trusting the `orgId` query parameter alone.

**R07 — Keyset Pagination Archive**
`GET /archive/telemetry`: Implement append-only long-term persistence with configurable retention. The `channel` filter is MANDATORY. Results MUST be ordered by `timestamp ASC` AND `id ASC`. Implement a composite index on `(well_id, channel, timestamp, id)`. The endpoint MUST accept a single `cursor` query parameter (Base64) that decodes the full composite key AND require the same filter shape used to generate it.

**R08 — Multi-Organization RBAC**
Replace facility scoping with a normalized join table `user_role_facility_scopes(user_role_id, facility_id)`. **Trust Link Constraint:** Resolve user identity AND organization context server-side using a seeded `api_token` mapping; MUST NOT rely on caller-supplied headers for identity. Require `AuthorizationService` checks AND audit log insertions on EVERY endpoint AND service that returns operational, historical, AND regulatory data.
...
- **Input (constructor):** `id: Long`, `wellId: Int`, `facilityId: Int`, `timestamp: String`, `sourceTimestamp: String`, `channel: String`, `value: Double`, `modemDiagnosticCode: String`, `commStatus: String`
...
- **Output:** N/A (data class)
- **Description:** Per-well daily production allocation result. The `allocationStatus` MUST be set to `"unallocatable"` if `Σ net_oil_at_point <= 0`, otherwise it MUST be set to `"allocated"`.
- **Decorators:** `@Serializable`

- **Path:** `src/main/kotlin/model/ChannelStatus.kt`
- **Name:** `ChannelStatus`
- **Type:** `@Serializable data class`
- **Input (constructor):** `wellId: Int`, `facilityId: Int`, `channel: String`, `signalCondition: String`, `connectivityCondition: String`, `probableCause: String?`, `lastSeen: String`
- **Output:** N/A (data class)
- **Description:** DTO for diagnostics.
- **Decorators:** `@Serializable`
...
- **Description:** Columns: `id`, `wellId`, `facilityId`, `timestamp`, `sourceTimestamp`, `channel`, `value`, `modemDiagnosticCode`. MUST include composite index on `(well_id, channel, timestamp, id)`.
...
- **Description:** Report generation service. MUST call AuthorizationService checks AND insert audit log rows for report access.
...
- **Description:** Seeds 72h telemetry AND 31 days of alarm history.
...
- **Type:** API Endpoint
- **Input:** Query params: `type: String` (MUST be exactly one of: `"federal_ghg"`, `"state_production"`, AND `"blm_lease"`), `startDate: String`, `endDate: String`. Header: `Authorization: String` (api_token).
- **Description:** MUST call AuthorizationService before generation.

- **Path:** `GET /archive/telemetry`
- **Name:** `getArchiveTelemetry`
- **Type:** API Endpoint
- **Input:** Query params: `wellId: Int` (required), `channel: String` (MANDATORY), `cursor: String?`. Header: `Authorization: String` (api_token).
