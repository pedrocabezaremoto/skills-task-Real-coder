# SCADA Data Aggregation and Operations Support Service - v1
...
Implement a telemetry ingestion module. Each reading MUST include: `well_id` (String), `timestamp` (ISO-8601 String), `channel` (Enum: `pressure`, `temperature`, `flow`, `gor`), `value` (Double), `transport` (Enum: `cellular`, `satellite`, `radio`), and `quality_flag` (Enum: `ok`, `flatline`, `signal_loss`). The system MUST distinguish `flatline` (value unchanged for 5+ consecutive readings) from `signal_loss` (no reading received for 10+ minutes). Store all readings in a `telemetry` table.

**R02 — Sample Dataset**
Include a bundled sample dataset generator. It MUST populate the database with: 50 wells, 3 sales delivery points, 1 gathering system, and 72 hours of telemetry at 5-minute intervals. The generator MUST execute automatically on first startup if the database is empty.
...
Implement a `GET /reports/regulatory` endpoint. Supported `type` parameters: `state_production`, `federal_ghg`, `blm_lease`.
- `state_production`: Returns monthly well-level net volumes.
- `federal_ghg`: Returns total methane/CO2 equivalent values.
- `blm_lease`: Returns production totals grouped by lease type (`federal`, `state`).

**R07 — Historical Archive**
Implement a `GET /archive/telemetry` endpoint. Parameters: `well_id` (String), `channel` (String), `from` (ISO-8601), `to` (ISO-8601). It MUST return all raw telemetry readings in the specified range, sorted by `timestamp` ascending.

**R08 — Access Control**
Implement role-based access (RBAC) with two roles: `operator` and `auditor`.
- `operator`: Full access to all endpoints.
- `auditor`: Read-only access (GET endpoints only).
Authentication MUST use HTTP Basic Auth. Hardcode two users: `operator_user` (role: `operator`) and `audit_user` (role: `auditor`), both with password `test1234`.
